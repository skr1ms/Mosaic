...
func (s *AdminService) DeletePartner(id uuid.UUID) error {
	...
	couponCount, _ := s.deps.CouponRepository.CountByPartnerID(context.Background(), id)
	articleCount := int64(0)
	if articles, err := s.deps.PartnerRepository.GetAllArticlesByPartner(context.Background(), id); err == nil {
		articleCount = int64(len(articles))
	}

	s.deps.Logger.Info().
		Str("partner_id", id.String()).
		Str("partner_code", partner.PartnerCode).
		Str("brand_name", partner.BrandName).
		Str("domain", partner.Domain).
		Int64("coupon_count", int64(couponCount)).
		Int64("article_count", articleCount).
		Msg("Starting partner deletion with cascade cleanup")
	...
	s.deps.Logger.Info().
		Str("partner_id", id.String()).
		Str("partner_code", partner.PartnerCode).
		Int64("deleted_coupons", int64(couponCount)).
		Int64("deleted_articles", articleCount).
		Msg("Partner successfully deleted with all associated data")
	...
	if s.deps.GitLabClient != nil && s.deps.GoroutineManager != nil && partner.Domain != "" {
		s.deps.GoroutineManager.StartGoroutineWithTimeout("trigger_domain_cleanup_pipeline", 30*time.Second, func() error {
			...
			if err != nil {
				s.deps.Logger.Error().
					Err(err).
					Str("partner_domain", partner.Domain).
					Msg("Failed to trigger domain cleanup pipeline")
				return fmt.Errorf("failed to trigger domain cleanup pipeline: %w", err)
			}
			s.deps.Logger.Info().
				Str("partner_domain", partner.Domain).
				Msg("Domain cleanup pipeline triggered successfully")
			return nil
		})
	}
