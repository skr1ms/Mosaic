...
func (handler *AdminHandler) generateAndDeployNginxConfig() error {
	partners, err := handler.deps.AdminService.GetActivePartnersWithDomains()
	if err != nil {
		return fmt.Errorf("failed to get partners: %w", err)
	}

	config, err := handler.generateNginxConfig(partners)
	if err != nil {
		return fmt.Errorf("failed to generate nginx config: %w", err)
	}

	err = handler.saveNginxConfig(config)
	if err != nil {
		return fmt.Errorf("failed to save nginx config: %w", err)
	}

	// CI/CD pipeline is now triggered automatically in UpdatePartnerDomain method
	return nil
}

func (handler *AdminHandler) generateNginxConfig(partners []partner.Partner) (string, error) {
	var partnerDomains []string
	for _, p := range partners {
		if p.Domain != "" && p.Status == "active" {
			partnerDomains = append(partnerDomains, p.Domain)
		}
	}
...
http {
	...
	// Add all partner domains to the HTTP redirect
	if len(partnerDomains) > 0 {
		baseConfig += " " + strings.Join(partnerDomains, " ")
	}
	...
	// Generate individual server blocks for each partner domain (PUBLIC SITE with branding)
	// Admin panel access remains ONLY on adm.doyoupaint.com
	for _, domain := range partnerDomains {
		partnerBlock := fmt.Sprintf(`

    # Partner domain: %s (Public Site with Branding)
    server {
        listen 443 ssl;
        http2 on;
        server_name %s;
        ...
        # Rate limiting for partner domains
        limit_req zone=general burst=20 nodelay;
        
        # Client settings
        client_max_body_size 100M;
        
        # WebSocket endpoints for partner chat
        location /api/ws/ {
            proxy_pass http://backend;
            proxy_http_version 1.1;
            proxy_set_header Upgrade $http_upgrade;
        ...
        # Public API endpoints (partner branded)
        location /api/ {
            limit_req zone=api burst=50 nodelay;
            
            # Handle CORS preflight requests
            if ($request_method = 'OPTIONS') {
                add_header Access-Control-Allow-Origin "https://%s" always;
                add_header Access-Control-Allow-Methods "GET, POST, PUT, DELETE, OPTIONS" always;
                add_header Access-Control-Allow-Headers "Authorization, Content-Type, X-Requested-With" always;
                add_header Access-Control-Allow-Credentials true always;
                add_header Access-Control-Max-Age 1728000;
        ...
        location /health {
            access_log off;
            return 200 "OK - Partner Domain: %s";
            add_header Content-Type text/plain;
        }
        
        # MinIO proxy for public images only
        location /mosaic-images/ {
            proxy_pass http://minio_s3/mosaic-images/;
            proxy_set_header Host $http_host;
            proxy_set_header X-Real-IP $remote_addr;
        ...
        # Partner branded public site (MAIN LOCATION)
        location / {
            proxy_pass http://public-site;
            proxy_set_header Host $host;
            proxy_set_header X-Real-IP $remote_addr;
...
func (handler *AdminHandler) saveNginxConfig(config string) error {
	configPath := "../../deployments/nginx/production.conf"
...
func (handler *AdminHandler) GenerateProductURL(c *fiber.Ctx) error {
	...
	// Validate parameters
	if req.Marketplace == "" || req.Style == "" || req.Size == "" {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"error": "Marketplace, style, and size are required",
		})
	}

	// Check that marketplace is correct
	validMarketplaces := []string{"ozon", "wildberries"}
	isValidMarketplace := false
	for _, valid := range validMarketplaces {
		if req.Marketplace == valid {
			isValidMarketplace = true
			break
		}
	}
	...
	// Get partner to check existence
	partner, err := handler.deps.AdminService.GetPartnerRepository().GetByID(c.Context(), partnerID)
	if err != nil {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
			"error": "Partner not found",
		})
	...
	// Generate URL directly using logic from partner service
	url := handler.generateProductURL(partner, req.Size, req.Style, req.Marketplace)

	// Get article for additional information
	article, err := handler.deps.AdminService.GetPartnerRepository().GetArticleBySizeStyle(
		c.Context(), partnerID, req.Size, req.Style, req.Marketplace)
...
// generateProductURL generates product URL for marketplace
func (handler *AdminHandler) generateProductURL(partner *partner.Partner, size, style, marketplace string) string {
	// Get article from database
	article, err := handler.deps.AdminService.GetPartnerRepository().GetArticleBySizeStyle(
		context.Background(), partner.ID, size, style, marketplace)
	if err != nil || article == nil || article.SKU == "" {
	...
	// Generate link based on template or standard patterns
	switch marketplace {
	case "ozon":
		return handler.generateOzonProductURL(partner, article, size, style)
	case "wildberries":
		return handler.generateWildberriesProductURL(partner, article, size, style)
	default:
		return ""
	}
}

// generateOzonProductURL generates link for Ozon
func (handler *AdminHandler) generateOzonProductURL(partner *partner.Partner, article *partner.PartnerArticle, size, style string) string {
	// First try to use partner template
	...
	// If no template, use standard Ozon pattern (same as public service)
	// Format: https://www.ozon.ru/product/{sku}
	return fmt.Sprintf("https://www.ozon.ru/product/%s", article.SKU)
}

// generateWildberriesProductURL generates link for Wildberries
func (handler *AdminHandler) generateWildberriesProductURL(partner *partner.Partner, article *partner.PartnerArticle, size, style string) string {
	// First try to use partner template
	...
	// If no template, use standard Wildberries pattern with detail.aspx
	// Format: https://www.wildberries.ru/catalog/{sku}/detail.aspx
