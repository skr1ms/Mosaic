...
type ImageServiceInterface interface {
	GetQueue(status string) ([]*Image, error)
...
type MosaicGeneratorInterface interface {
	CreateMosaicGrid(img interface{}, size, style string) (*MosaicGrid, error)
	CreateColorPalette(style string) []MosaicColor
	FindClosestColor(r, g, b uint8, palette []MosaicColor) MosaicColor
	CalculateGridDimensions(size string) (width, height int)
}

type PDFGeneratorInterface interface {
	GenerateMosaicPDF(grid *MosaicGrid, size, couponCode string) ([]byte, error)
	DrawMiniGrid(grid *MosaicGrid) error
}

type ImageProcessorInterface interface {
	ApplyImageEditing(img interface{}, params ImageEditParams) interface{}
	CreatePreview(imageData []byte, couponID uuid.UUID) (string, error)
	ResizeImage(img interface{}, width, height int) interface{}
	RotateImage(img interface{}, rotation int) interface{}
	CropImage(img interface{}, x, y, width, height int) interface{}
	ScaleImage(img interface{}, scale float64) interface{}
}
...
type ConfigInterface interface {
	...
	GetStableDiffusionConfig() StableDiffusionConfig
}
...
type S3Config struct {
	...
	URLExpiry       time.Duration `json:"url_expiry"`
