	...
	tests := []struct {
		...
		},
		{
			...
			mockSetup: func(repo *MockPaymentRepository, couponRepo *MockCouponRepository, partnerRepo *MockPartnerRepository, config *MockConfig) {
				...
				couponRepo.On("CodeExists", mock.Anything, mock.Anything).Return(false, nil)
				couponRepo.On("Create", mock.Anything, mock.Anything).Return(nil)
				repo.On("UpdateOrderCoupon", mock.Anything, order.OrderNumber, mock.Anything).Return(nil)
				config.On("GetAlfaBankConfig").Return(AlphaBankConfig{WebhookSecret: ""})
			},
			expectedError: false,
		},
	...
	tests := []struct {
	...
	}{
		{
			...
			mockSetup: func(couponRepo *MockCouponRepository, partnerRepo *MockPartnerRepository, paymentRepo *MockPaymentRepository) {
				partnerRepo.On("GetByID", mock.Anything, mock.Anything).Return(createTestPartner(), nil)
				couponRepo.On("CodeExists", mock.Anything, mock.Anything).Return(false, nil)
				couponRepo.On("Create", mock.Anything, mock.Anything).Return(nil)
				paymentRepo.On("UpdateOrderCoupon", mock.Anything, mock.Anything, mock.Anything).Return(nil)
			},
			expectedError: false,
		},
		{
			...
			mockSetup: func(couponRepo *MockCouponRepository, partnerRepo *MockPartnerRepository, paymentRepo *MockPaymentRepository) {
				partnerRepo.On("GetByID", mock.Anything, mock.Anything).Return(createTestPartner(), nil)
				couponRepo.On("CodeExists", mock.Anything, mock.Anything).Return(false, nil)
				couponRepo.On("Create", mock.Anything, mock.Anything).Return(errors.New("creation failed"))
			},
			expectedError: true,
		},
	}
...
func TestPaymentService_GenerateUniqueCouponCode(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "successful_code_generation",
			partnerCode: "TEST",
		...
		{
			name:        "code_exists_retry_success",
			partnerCode: "TEST",
			mockSetup: func(couponRepo *MockCouponRepository) {
				couponRepo.On("CodeExists", mock.Anything, mock.Anything).Return(true, nil).Once()
				couponRepo.On("CodeExists", mock.Anything, mock.Anything).Return(false, nil).Once()
			},
			expectedError: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := service.generateUniqueCouponCode(tt.partnerCode)
...
func TestPaymentService_CompletePaymentFlow(t *testing.T) {
	t.Run("end_to_end_successful_payment", func(t *testing.T) {
		...
		mockPaymentRepo.On("UpdateOrderCoupon", mock.Anything, mock.Anything, mock.Anything).Return(nil)
