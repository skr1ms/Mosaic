...
func TestCouponService_ActivateCoupon(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			couponID: uuid.New(),
			zipURL:   "http://example.com/materials.zip",
			mockSetup: func(repo *MockCouponRepository, redis *MockRedisClient) {
				repo.On("ActivateCoupon", mock.Anything, mock.AnythingOfType("uuid.UUID"), "http://example.com/materials.zip").Return(nil)
			},
			expectedError: false,
		},
		{
			...
			zipURL:   "http://example.com/materials.zip",
			mockSetup: func(repo *MockCouponRepository, redis *MockRedisClient) {
				repo.On("ActivateCoupon", mock.Anything, mock.AnythingOfType("uuid.UUID"), "http://example.com/materials.zip").Return(errors.New("database error"))
			},
			expectedError: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := service.ActivateCoupon(tt.couponID, tt.zipURL)
