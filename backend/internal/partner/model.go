...
type Partner struct {
	...
	// Прим.: шаблон может содержать {sku}, {size}, {style}
	...
	// Реляция: артикулы/товары партнёра
	Articles []*PartnerArticle `bun:"rel:has-many,join:id=partner_id" json:"articles"`
}

// PartnerArticle представляет артикул товара партнера для конкретного размера и стиля
type PartnerArticle struct {
	...
	// Ключевые поля для сетки
	Size        string `bun:"size,notnull" json:"size"`               // 20x20, 30x40, 40x40, 40x50, 40x60, 50x70
	Style       string `bun:"style,notnull" json:"style"`             // grayscale, skin_tones, pop_art, max_colors
	Marketplace string `bun:"marketplace,notnull" json:"marketplace"` // ozon, wildberries

	// Артикул для конкретной ячейки
	SKU string `bun:"sku" json:"sku"` // может быть пустым если ячейка не заполнена
...
// Константы для размеров, стилей и маркетплейсов
const (
	// Размеры (колонки)
	Size20x20 = "20x20"
	Size30x40 = "30x40"
	...
	// Стили (строки) - из stableDiffusion клиента
	StyleGrayscale = "grayscale"  // Grayscale
	StyleSkinTones = "skin_tones" // Skin tones
	StylePopArt    = "pop_art"    // Pop art
	StyleMaxColors = "max_colors" // Maximum colors
...
var (
	AvailableSizes  = []string{Size20x20, Size30x40, Size40x40, Size40x50, Size40x60, Size50x70}
