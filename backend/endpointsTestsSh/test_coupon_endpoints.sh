...
echo "3. Получение статистики по купонам:"
...
echo "7. Экспорт купонов (показать заголовки):"
curl -s -I "$BASE_URL/coupons/export"
echo ""
...
COUPONS_LIST=$(curl -s "$BASE_URL/coupons/")
COUPON_ID=$(echo "$COUPONS_LIST" | jq -r '.coupons[0].id // empty')
COUPON_CODE=$(echo "$COUPONS_LIST" | jq -r '.coupons[0].code // empty')
...
if [ -n "$COUPON_ID" ] && [ "$COUPON_ID" != "null" ] && [ "$COUPON_ID" != "empty" ]; then
  ...
  echo "  d) Попытка активации купона (нужен email):"
