  ...
  return (
    ...
    >
      <div className="p-6 sm:p-8 flex flex-col h-full">
        <div className={`w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-gradient-to-r ${active ? 'from-brand-primary to-brand-secondary' : gradient} flex items-center justify-center text-white mb-4 sm:mb-6 mx-auto`}>
          {icon}
        </div>
          ...
          {disabled && comingSoon ? (
            ...
            buttonText && (
              <button
                onClick={onClick}
                disabled={disabled}
                className={`w-full py-3 px-4 sm:px-6 rounded-lg font-semibold text-base sm:text-lg transition-all duration-200 flex items-center justify-center space-x-2 min-h-[48px] focus:ring-2 focus:ring-offset-2 ${
                  active 
                    ? `bg-gradient-to-r from-brand-primary to-brand-secondary text-white hover:from-brand-primary/90 hover:to-brand-secondary/90 hover:shadow-lg focus:ring-brand-primary`
