...
const SupportChatWidget = () => {
  ...
  const [menu, setMenu] = React.useState({ visible: false, x: 0, y: 0, msg: null })
  ...
  const fetchMessages = async (c) => {
    ...
    if (!current) return
    try {
      const list = await MosaicAPI.getSupportMessages(current.chat_id, current.access_token)
      setMessages(list)
    } catch (err) {
      if (err?.status === 404) {
        // Чат не найден — очищаем локальное состояние, НЕ создаём новый автоматически
        try { sessionStorage.removeItem('support:chat') } catch {}
        setChat(null)
        setMessages([])
      } else if (err?.status === 401) {
        // Токен невалидный - очищаем и пересоздаем чат
        try { sessionStorage.removeItem('support:chat') } catch {}
        setChat(null)
        setMessages([])
      }
    ...
    const init = async () => {
      try {
        ...
        if (chatData) {
          ...
          // не создаём чат до первой отправки сообщения
          setChat(null)
          setMessages([])
        }
      } catch (e) {
  ...
  // Realtime WS: subscribe with guest token; append incoming messages
  React.useEffect(() => {
    if (!open || !chat) return
    ...
    try {
      ...
      let origin
      try {
        // if full URL like http://host:port/api
        origin = new URL(apiBase).origin
      } catch {
        // relative '/api' → use current origin
        ...
        try {
          ...
          if (envelope?.type === 'support_new_message' && envelope.data) {
            ...
            if (String(m.chat_id || chat.chat_id) === String(chat.chat_id)) {
              ...
              // ignore messages of other chats
            }
          }
          ...
          if (envelope?.type === 'support_message_delete' && envelope.data) {
            ...
            if (String(payload.chat_id || chat.chat_id) === String(chat.chat_id)) {
  ...
  const reinitChat = async () => {
    try {
      try { sessionStorage.removeItem('support:chat') } catch {}
      // не создаём автоматически новый чат здесь; вернём null
      setChat(null)
      setMessages([])
      return null
    } catch (e) {
  ...
  const send = async () => {
    // Ensure chat exists (create on first send)
    let currentChat = chat
    if (!currentChat) {
      try {
        const fresh = await MosaicAPI.startSupportChat('')
        try { sessionStorage.setItem('support:chat', JSON.stringify(fresh)) } catch {}
        setChat(fresh)
        currentChat = fresh
      } catch (e) {
    ...
    try {
      if (editingId && text) {
        await MosaicAPI.updateSupportMessage(editingId, text, currentChat.access_token)
        setMessages(prev => prev.map(m => m.id === editingId ? { ...m, content: text, edited: true } : m))
        cancelEdit()
      ...
      if (hasFiles) {
        let baseId = null
              ...
              const created = await MosaicAPI.sendSupportMessage(currentChat.chat_id, contentText, currentChat.access_token)
          baseId = created?.id
          setValue('')
          if (textareaRef.current) textareaRef.current.style.height = 'auto'
        } catch (e) {
          if (e?.status === 404) {
            ...
            if (fresh) {
              try {
                const contentText = text || ''
                const created2 = await MosaicAPI.sendSupportMessage(fresh.chat_id, contentText, fresh.access_token)
                baseId = created2?.id
                currentChat = fresh
              } catch (_) {}
            }
        ...
        if (baseId) {
          for (const f of selectedFiles) {
            try {
              ...
              fd.append('file', f)
              const token = currentChat?.access_token || (sessionStorage.getItem('support:chat') ? (JSON.parse(sessionStorage.getItem('support:chat')||'{}').access_token||'') : '')
              console.log('Uploading attachment:', f.name, 'to baseId:', baseId, 'with token:', token?.substring(0, 10) + '...')
              console.log('File size:', f.size, 'bytes, type:', f.type)
              
              const response = await fetch(`${(import.meta.env.VITE_API_BASE_URL || '/api')}/public/support/messages/${baseId}/attachments`, {
                method: 'POST',
                body: fd,
                headers: { 
      ...
      let created = null
      try {
        created = await MosaicAPI.sendSupportMessage(currentChat.chat_id, text, currentChat.access_token)
      } catch (e) {
        if (e?.status === 404) {
          ...
          try { sessionStorage.setItem('support:chat', JSON.stringify(fresh)) } catch {}
          setChat(fresh)
          created = await MosaicAPI.sendSupportMessage(fresh.chat_id, text, fresh.access_token)
          currentChat = fresh
        } else if (e?.status === 401) {
          try { sessionStorage.removeItem('support:chat') } catch {}
          setChat(null)
      ...
      setValue('')
      if (textareaRef.current) textareaRef.current.style.height = 'auto'
      // WS доставит событие остальным подписчикам; локально уже отрисовали через upsert
    } catch (e) {
  ...
  const handleAttachClick = async () => {
    // Ensure chat exists before selecting a file
    if (!chat) {
      try {
        const fresh = await MosaicAPI.startSupportChat('')
        try { sessionStorage.setItem('support:chat', JSON.stringify(fresh)) } catch {}
  ...
  React.useEffect(() => {
    ...
    const onKey = (ev) => { if (ev.key === 'Escape') setMenu(m => ({ ...m, visible: false })) }
  ...
  return (
    <div>
      {menu.visible && menu.msg && (
        <div
          style={{ position: 'fixed', top: menu.y, left: menu.x, width: 160, zIndex: 2000 }}
          className="support-context-menu"
          onClick={(e) => e.stopPropagation()}
        >
          <button
            onClick={() => { setMenu({ visible: false, x: 0, y: 0, msg: null }); setEditingId(menu.msg.id); setValue(menu.msg.content || ''); setTimeout(() => { if (textareaRef.current) { textareaRef.current.focus(); textareaRef.current.style.height = 'auto'; textareaRef.current.style.height = Math.min(textareaRef.current.scrollHeight, 200) + 'px'; } }, 0); }}
          >
      ...
      {open && (
        <div style={{ position: 'fixed', right: 20, bottom: 100, width: 560, height: 560, zIndex: 1000 }} className="support-chat-window bg-white rounded-xl shadow-xl border flex flex-col">
          <div className="px-4 py-3 border-b font-semibold flex items-center justify-between bg-gray-50 rounded-t-xl">
            ...
            <button className="text-gray-500 hover:text-gray-700 text-xl font-normal" onClick={() => setOpen(false)} title={t('support_chat.close')}>×</button>
          </div>
          <div className="flex-1 overflow-auto p-4 space-y-3 support-chat-messages" onScroll={() => {}}>
            {messages.map(m => {
              const mine = m.sender_role !== 'admin' && m.sender_role !== 'main_admin'
              
              // Извлекаем имя файла из URL вложения (как в админке)
              let displayName = '';
              ...
              return (
                <div key={m.id} style={{ display: 'flex', justifyContent: mine ? 'flex-end' : 'flex-start' }}>
                  <div className={`support-message-bubble ${mine ? 'mine' : 'theirs'}`} onContextMenu={(e) => openContextMenu(e, m)}>
                      ...
                      <div className={`${contentText && !isDuplicate ? 'mt-2' : ''}`}>
                        <a 
                          ...
                          style={{ 
                            ...
                            backgroundColor: mine ? 'rgba(255,255,255,0.1)' : 'rgba(59,130,246,0.1)'
                          }}
                        >
                          📎 {displayName || t('support_chat.view_attachment')}
                        </a>
                    ...
                    <div className="support-message-meta">
                      <span className="support-message-time">
                        {new Date(m.timestamp || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: false })}
                        {m.edited ? ` · ${t('support_chat.edited')}` : ''}
                      </span>
                      {mine && (
                        <span
                          className="support-message-status"
                          title={m.read ? t('support_chat.message_read', 'Прочитано') : t('support_chat.message_sent', 'Отправлено')}
                        >
                          {m.read ? '✓✓' : '✓'}
                        </span>
                      )}
          ...
          <div className="p-4 border-t flex flex-col gap-3 bg-gray-50 rounded-b-xl">
            <div className="flex items-start gap-3">
              ...
              <textarea
                ref={textareaRef}
                value={value}
                placeholder={editingId ? t('support_chat.edit_placeholder') : t('support_chat.message_placeholder')}
                className="support-message-input flex-1"
                rows={1}
                style={{ resize: 'none', minHeight: 40, maxHeight: 160, overflowY: 'auto' }}
                onChange={(e) => {
              ...
              >
                {editingId ? t('support_chat.save_button') : t('support_chat.send_button')}
              </button>
            </div>
