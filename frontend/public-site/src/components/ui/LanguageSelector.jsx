...
const LanguageSelector = () => {
  ...
  return (
    <div className="relative group">
      <button className="flex items-center space-x-1 sm:space-x-2 px-2 sm:px-3 py-2 text-gray-700 hover:text-gray-900 transition-colors">
      ...
      <div className="absolute right-0 mt-2 w-32 sm:w-48 bg-white rounded-md shadow-lg border border-gray-200 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 max-w-[calc(100vw-2rem)]">
        {languages.map((language) => (
          <button
            ...
            className={`w-full text-left px-3 sm:px-4 py-2 text-sm hover:bg-gray-100 transition-colors ${
              i18n.language === language.code ? 'bg-brand-primary/10 text-brand-primary' : 'text-gray-700'
            }`}
          >
            <span className="mr-1 sm:mr-2">{language.flag}</span>
            <span className="hidden sm:inline">{language.name}</span>
            <span className="sm:hidden text-xs">{language.code.toUpperCase()}</span>
          </button>
        ))}
      </div>
    </div>
