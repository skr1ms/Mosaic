...
import { CheckCircle, AlertTriangle, Download, Mail, Loader } from 'lucide-react'
import { useNavigate } from 'react-router-dom'
import { useMutation } from '@tanstack/react-query'
import { MosaicAPI } from '../../api/client'
import { useUIStore } from '../../store/partnerStore'

const SchemaGenerator = ({ imageId, selectedOptions, onBack, onComplete, onLockNavigation }) => {
  ...
  const handleGenerateSchema = async () => {
    try {
      ...
      try { sessionStorage.setItem(`editor:confirmed:${imageId}`, '1') } catch {}
      ...
      const mapParams = (opts) => {
        const styleMap = { original: { style: 'grayscale', use_ai: false }, enhanced: { style: 'max_colors', use_ai: true } }
        const lightingMap = { natural: 'sun', moonlight: 'moon', venus: 'venus' }
      ...
      const waitForCompleted = async () => {
        ...
        while (Date.now() - start < TIMEOUT_MS) {
          ...
          if (status?.status === 'failed') throw new Error(status?.error_message || 'Generation failed')
          await new Promise(r => setTimeout(r, 2000))
        }
      ...
      addNotification({ type: 'success', title: t('notifications.schema_generated'), message: t('notifications.schema_generated_desc') })
    } catch (error) {
      setIsGenerating(false)
      if (onLockNavigation) onLockNavigation(false)
      addNotification({ type: 'error', title: t('notifications.schema_error'), message: error.message || t('notifications.schema_error_desc') })
    }
  }
  ...
  useEffect(() => {
    try {
      const confirmed = sessionStorage.getItem(`editor:confirmed:${imageId}`) === '1'
      const savedSchemaData = sessionStorage.getItem(`editor:schemaData:${imageId}`)
      if (confirmed) {
  ...
  useEffect(() => {
    if (schemaData) {
      try {
        sessionStorage.setItem(`editor:schemaData:${imageId}`, JSON.stringify(schemaData))
      } catch {}
    }
  }, [schemaData, imageId])
      ...
      <AnimatePresence mode="wait">
        {showWarning && !isGenerating && !schemaData && (
          <motion.div
            ...
            className="bg-brand-accent/5 border border-brand-accent/20 rounded-xl p-6"
          >
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                <h4 className="text-lg font-semibold text-brand-accent mb-2">
                  {t('editor.confirmation.warning_title')}
                </h4>
                <p className="text-brand-accent/80 mb-2">
          ...
          >
            ...
            <div className="max-w-2xl mx-auto bg-gray-50 rounded-xl p-6 mb-8">
              ...
              <div className="grid md:grid-cols-3 gap-4 text-sm">
                <div>
                  <span className="font-medium text-gray-700">{t('editor.confirmation.style')}:</span>
                  <p className="text-gray-900">{t(`editor.styles.${selectedOptions.style}.title`)}</p>
                </div>
                <div>
                  <span className="font-medium text-gray-700">{t('editor.confirmation.lighting')}:</span>
                  <p className="text-gray-900">{t(`editor.lighting.${selectedOptions.lighting}.title`)}</p>
                </div>
                <div>
                  <span className="font-medium text-gray-700">{t('editor.confirmation.contrast')}:</span>
                  <p className="text-gray-900">{t(`editor.contrast.${selectedOptions.contrast}.title`)}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-center space-x-4">
              ...
              <button
                ...
                className="px-8 py-3 bg-brand-primary text-white rounded-lg hover:bg-brand-primary/90 transition-colors"
              >
        ...
        {isGenerating && (
          ...
          >
            ...
            <div className="max-w-md mx-auto">
              <div className="w-24 h-24 border-4 border-brand-primary/20 border-t-brand-primary rounded-full animate-spin mx-auto mb-6" />
            ...
            <div className="flex justify-center mb-8">
              <div className="grid md:grid-cols-2 gap-6 max-w-2xl">
                <motion.button
                  ...
                  className="flex flex-col items-center p-6 bg-brand-primary/5 rounded-xl hover:bg-brand-primary/10 transition-colors"
                >
                  <Download className="w-12 h-12 text-brand-primary mb-4" />
                ...
                <motion.button
                  ...
                  className="flex flex-col items-center p-6 bg-brand-secondary/5 rounded-xl hover:bg-brand-secondary/10 transition-colors"
                >
                  <Mail className="w-12 h-12 text-brand-secondary mb-4" />
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">
                  ...
                  <form onSubmit={handleSendEmail} className="space-y-4">
                    ...
                    <div className="flex space-x-3">
                      ...
                      <button
                        ...
                        className="flex-1 px-4 py-2 bg-brand-secondary text-white rounded-lg hover:bg-brand-secondary/90 disabled:opacity-50 transition-colors"
                      >
