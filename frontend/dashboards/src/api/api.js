...
function checkTokenOnInit() {
  ...
  // Если нет токена вообще
  if (!token) {
    return false;
  }
  
  // Проверяем, что это новая сессия браузера
  const sessionId = sessionStorage.getItem('auth_session_id');
  const storedSessionId = localStorage.getItem('auth_session_id');
  
  // Если это новая сессия браузера, требуем повторную аутентификацию
  ...
  if (token && isTokenExpired(token)) {
    ...
    // Если refresh токен тоже истек или отсутствует - требуем логин
    ...
    // Даже если refresh токен валиден, требуем повторный логин для безопасности
    console.warn('Access token expired, requiring re-login for security');
    clearAuth();
    return false;
  }
  
  // Токен валиден, но проверяем его недавность (дополнительная безопасность)
  const decoded = decodeJWT(token);
  if (decoded && decoded.iat) {
    const tokenAge = Math.floor(Date.now() / 1000) - decoded.iat;
    // Если токену больше 30 минут, требуем повторный логин
    if (tokenAge > 1800) { // 30 минут
    ...
    if (token && isTokenExpired(token)) {
      ...
      if (refreshToken && !isTokenExpired(refreshToken)) {
        try {
          ...
          clearAuth();
          // Всегда перенаправляем на страницу логина при ошибке аутентификации
        ...
        clearAuth();
        // Всегда перенаправляем на страницу логина при истечении токенов
        if (window.location.pathname !== '/login') {
          window.location.href = '/login';
        }
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (status === 401 && !isLoginRequest) {
      ...
      // Очистка и редирект при неуспешном обновлении
      clearAuth();
