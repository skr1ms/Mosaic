...
const UserBox = () => {
  ...
  const [emailRetried, setEmailRetried] = useState(false);
  ...
  const handleLogout = () => {
    ...
    // Перенаправляем на страницу логина для HashRouter
  ...
  // удалено управление видимостью email – email всегда видим
        ...
        <ModalFooter>
          ...
          <Button color="secondary" onClick={() => setShowChangePassword(false)}>
            {t('common.cancel')}
              ...
              {/* {t('userbox.create_administrator_modal')} (только для admin) */}
