    ...
    useEffect(() => {
        const loadPartners = async () => {
            try {
                ...
                // Находим реального партнёра с кодом 0000 и исключаем его из выпадающего списка
                const own = list.find(p => p?.partner_code === '0000');
                setOwnPartnerId(own?.id || null);
                const filtered = list.filter(p => p?.partner_code !== '0000');
                // Добавляем единственную опцию «Собственные купоны (0000)»
                setPartners([
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data?.codes) {
                // Преобразуем коды в формат для отображения
