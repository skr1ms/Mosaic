...
// Без внешних библиотек масок: собственная маска ввода даты/времени DD.MM.YYYY HH:mm
import {
...
const ManageCoupons = () => {
    ...
    const [exportFormat, setExportFormat] = useState('txt'); // 'txt' | 'csv'
    const [exportScope, setExportScope] = useState('all'); // 'all' | 'by_partner'
    ...
    const loadPartners = async () => {
        try {
            ...
            // Исключаем реального партнёра с кодом 0000, оставляем только алиас
    ...
    const loadCoupons = async () => {
        try {
            ...
            // Поиск по коду купона или партнер коду
            if (searchTerm && searchTerm.trim()) {
                params.append('search', searchTerm.trim());
            }
            // Статусы и блокировка: 'new' | 'activated' | 'used' | 'completed' | 'blocked'
    ...
    useEffect(() => {
        loadCoupons();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [statusFilter, sizeFilter, styleFilter, partnerFilter, page, limit, createdFrom, createdTo, usedFrom, usedTo, searchTerm]);

    // Поскольку поиск теперь происходит на бэкенде, используем coupons напрямую
    ...
    const handleDownloadMaterials = async (coupon) => {
        try {
            // Проверяем наличие прямой ссылки на схему
            if (coupon?.schemaUrl) {
            ...
            // Fallback: используем admin API для скачивания материалов купона
    ...
    return (
        <Fragment>
            ...
            {/* Подтверждение удаления одного купона */}
            ...
            {/* Подтверждение удаления выбранных купонов */}
            ...
            {/* Подтверждение сброса выбранных купонов */}
