...
const PartnerArticlesGrid = () => {
  ...
  const fetchArticles = useCallback(async () => {
    try {
      ...
      // Convert nested structure to flat object for easy access
      // API returns: marketplace -> style -> size -> sku
      const articlesMap = {}
  ...
  const handleSaveCell = async () => {
    ...
    try {
      ...
      // Mark cell as saved (for green highlighting)
      if (sku) {
  ...
  const handleKeyNavigation = (e, currentMarketplace, currentStyle, currentSize) => {
    if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Tab'].includes(e.key)) {
      ...
      if (hasChanged) {
        // Update focused cell state immediately
                                ...
                                <div className="flex items-center space-x-1">
                                  <input
                                    ...
                                    onKeyDown={(e) => {
                                      ...
                                      // Only allow arrow navigation if not moving cursor in input
                                      if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
                                        // Check if cursor is at beginning/end of input for left/right arrows
