...
const ArticleGrid = ({ partnerId, isReadOnly = false }) => {
        ...
        try {
            ...
            if (localChanges) {
                try {
                    ...
                    // Применяем локальные изменения к загруженным данным
    ...
    useEffect(() => {
        if (partnerId && !isReadOnly) {
            ...
            // Если нет partnerId (создание партнера), убираем loading
            setLoading(false);
        }
    ...
    const handleSKUChange = (marketplace, style, size, value) => {
        ...
        if (isCreationMode) {
            ...
            // Для режима редактирования сохраняем изменения как плоскую структуру
            const currentArticles = JSON.parse(localStorage.getItem(`partner_articles_${partnerId}`) || '{}');
            const cellKey = `${marketplace}-${style}-${size}`;
            // Всегда сохраняем значение (включая пустые для удаления)
    ...
    // For creation mode (isReadOnly), we show the grid but don't save to server
