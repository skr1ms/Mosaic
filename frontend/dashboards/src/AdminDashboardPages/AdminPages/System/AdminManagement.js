...
const AdminManagement = () => {
    ...
    const fetchCurrentUserRole = async () => {
        try {
            // Сначала пытаемся получить роль из localStorage (если пользователь уже залогинен)
            ...
            // Если в localStorage нет, пытаемся получить из API
            ...
            if (response.data && response.data.current_user) {
                ...
                // Если API не возвращает роль, устанавливаем по умолчанию
                setCurrentUserRole('admin');
            }
        } catch (err) {
            console.error('Failed to fetch current user role:', err);
            // Пытаемся получить роль из localStorage как fallback
            const roleFromStorage = localStorage.getItem('userRole');
    ...
    return (
        <div className="admin-management">
            ...
            {/* Модальное окно обновления пароля */}
            ...
            {/* Модальное окно обновления email */}
