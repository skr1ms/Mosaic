...
const ImageQueue = () => {
  ...
  useEffect(() => { loadImages(); /* eslint-disable-next-line */ }, [status, page, limit, dateFrom, dateTo]);
  ...
  const openDetails = async (img) => {
    ...
    try {
      ...
      setDetails(res.data || {});
      console.log('Task details:', res.data); // Отладочный лог
    } catch (_) {
      setDetails(img);
    }
    try {
      ...
      setDetailsStatus(st);
      console.log('Task status:', st); // Отладочный лог
    } catch (_) {
  ...
  const fetchStatus = async (id) => {
    try {
      // Используем админский endpoint для получения деталей задачи
      const res = await api.get(`/admin/queue/${id}`);
      const taskData = res.data;
      
      // Формируем объект статуса с данными из админского endpoint
      return {
  ...
  const filtered = useMemo(() => {
    ...
    return (images || []).filter((it) => {
      // Фильтр по тексту (клиентская фильтрация для поиска)
  ...
  // Форматирование времени в 24ч формате по МСК
                          ...
                          <td>
                            <div className="btn-group">
                              ...
                              <Button
                                ...
                                onClick={async () => {
                                  try {
                                    // Используем API с токеном авторизации для скачивания схемы
