...
// Функция для очистки аутентификации (импортируем из api.js если нужно, или дублируем здесь)
function clearAuth() {
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Принудительная перезагрузка страницы для корректной инициализации
      setTimeout(() => {
...
const ForgotPasswordForm = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const loadReCaptcha = async () => {
      try {
        ...
        // Функция обратного вызова для рендеринга reCAPTCHA
        ...
        // Добавляем скрипт reCAPTCHA, если не добавлен
        if (!document.querySelector('script[src*="google.com/recaptcha/api.js"]')) {
          ...
          // Скрипт уже загружен, рендерим напрямую
          window.onRecaptchaLoad();
        }
      } catch (error) {
  ...
  }, []); // Убираем widgetId из зависимостей, чтобы избежать повторной загрузки
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(msg);
      // При ошибке также сбрасываем reCAPTCHA
      try { 
        if (window.grecaptcha && widgetId !== null) {
