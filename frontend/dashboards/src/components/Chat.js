...
const Chat = () => {
  ...
  const [confirmBlock, setConfirmBlock] = useState({ open: false, mode: 'block' });
  const [templatesOpen, setTemplatesOpen] = useState(false);
  const [unreadBySender, setUnreadBySender] = useState({});
  const [lastActiveMap, setLastActiveMap] = useState({}); // { userId: timestamp }
  // превью последних сообщений убраны по бизнес-логике
  ...
  useEffect(() => {
    ...
    const userName = (userRole === 'admin' || userRole === 'main_admin') ? 'Administrator' : 'Partner';
  ...
  const reorderByActivity = useCallback((list) => {
    try {
      return [...(list || [])].sort((a, b) => {
        const av = Number(lastActiveMap[a.id] || 0);
        const bv = Number(lastActiveMap[b.id] || 0);
        if (bv !== av) return bv - av;
        // стабильная сортировка: если активность равна, сохраняем текущий порядок
        return 0;
      });
    } catch { return list || []; }
  }, [lastActiveMap]);

  const bumpUserActivity = useCallback((userId) => {
    if (!userId) return;
    setLastActiveMap((prev) => ({ ...prev, [userId]: Date.now() }));
    // пересортировать текущие списки
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const response = await api.get(`/chat/users?role=${currentUser.role}${qs}`);
      ...
      if (currentUser.role === 'admin' || currentUser.role === 'main_admin') {
        try {
          ...
          // Дедуп по гостю: показываем только самый новый чат на гостя
          const uniqueByGuest = [];
  ...
  // Автообновление списка пользователей/чатов раз в 3 секунды
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // загрузка превью последних сообщений по собеседникам
    // превью последних сообщений убраны по бизнес-логике
  ...
  // Пересчитываем общий счетчик при изменении individual счетчиков
  useEffect(() => {
    setUnreadCount(partnerUnreadCount + supportUnreadCount);
  }, [partnerUnreadCount, supportUnreadCount]);

  // Авто-ресайз по изменению значения (вверх/вниз)
  useEffect(() => {
  ...
  // Автоскролл при выделении текста в контейнере сообщений
  useEffect(() => {
    const onMouseMove = (e) => {
    ...
    const tick = () => {
      try {
        ...
        if (container && selectingRef.current) {
          ...
          const margin = 96; // ширина зоны автоскролла у краёв
          let delta = 0;
          if (y > rect.bottom - margin && y < rect.bottom + margin) {
            const k = (y - (rect.bottom - margin)) / margin; // 0..1
            delta = 8 + Math.ceil(32 * k); // 8..40 px/frame
          } else if (y < rect.top + margin && y > rect.top - margin) {
            const k = ((rect.top + margin) - y) / margin; // 0..1
            delta = - (8 + Math.ceil(32 * k)); // -8..-40 px/frame
          }
          if (delta !== 0) {
  ...
  // Автообновление текущего диалога для подтягивания статусов прочитанности
  useEffect(() => {
  ...
  // Перенесено выше, чтобы не было обращения до инициализации (см. правку ниже)

  const fetchMessages = async (targetUserId) => {
    if (!currentUser || !targetUserId) return;
    try {
      if (selectedUser && selectedUser.role === 'support') {
        const response = await api.get('/support/messages', { params: { chat_id: targetUserId } });
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (selectedUser.role === 'support') {
        // Support chat HTTP flow (no WS)
        if (hasFiles) {
          let baseId = null;
          try {
            const contentText = text || '';
            const createRes = await api.post('/support/messages', { chat_id: selectedUser.id, content: contentText });
            baseId = String(createRes.data?.message?.id || createRes.data?.data?.id || '');
            setNewMessage('');
          ...
          if (baseId) {
            for (const f of selectedFiles) {
              try {
                ...
                fd.append('file', f);
                await api.post(`/support/messages/${baseId}/attachments`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
              } catch (error) {
        ...
        const response = await api.post('/support/messages', { chat_id: selectedUser.id, content: text });
      ...
      if (hasFiles) {
        // Создаем одно базовое сообщение
        let baseId = null;
        try {
          // For attachment-only messages, use descriptive placeholder text or empty content
          const contentText = text || '';
          const createRes = await api.post('/chat/messages', { target_id: selectedUser.id, content: contentText });
        ...
        if (baseId) {
          for (const f of selectedFiles) {
            try {
              const fd = new FormData();
              fd.append('file', f);
              console.log('Uploading attachment:', f.name, 'to baseId:', baseId);
              console.log('File size:', f.size, 'bytes, type:', f.type);
              
              const response = await api.post(`/chat/messages/${baseId}/attachments`, fd, { 
          ...
          // Обновим список сообщений, чтобы подтянуть attachmentUrl
          setTimeout(() => fetchMessages(selectedUser.id), 300);
        }
        setSelectedFiles([]);
        if (fileInputRef.current) fileInputRef.current.value = '';
      ...
      const payload = { type: 'message', data: { target_id: selectedUser.id, content: text } };
      if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
        // Отправляем через WS и ждём эхо от сервера, чтобы избежать дублей
  ...
  const wsUpdateMessage = (id, content) => {
    if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
      wsRef.current.send(JSON.stringify({ type: 'update', data: { id: Number(id), content } }));
  ...
  const wsDeleteMessage = (id) => {
    if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
      wsRef.current.send(JSON.stringify({ type: 'delete', data: { id: Number(id) } }));
      return Promise.resolve();
    }
  ...
  const refreshUnreadCount = async () => {
    try {
      // Получаем счетчик обычных сообщений (партнеры)
  ...
  const refreshSupportUnreadCount = async () => {
    // Только для админов
  ...
  const refreshUnreadBySender = async () => {
    try {
      ...
      const map = {};
      list.forEach((row) => { map[String(row.sender_id || row.senderId)] = Number(row.count) || 0; });
  ...
  const connectWebSocket = useCallback(() => {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (envelope?.type === 'support_new_message' && envelope.data) {
            ...
            // если сообщение не от текущего админа — увеличим support счетчик  
          ...
          if (envelope?.type === 'message' && envelope.data) {
            ...
            const curId = currentUserIdRef.current;
            const isForCurrentDialog = !!selId && (msg.senderId === selId || msg.targetId === selId);
            // Обновляем активность собеседника, как в мессенджерах
            const otherId = msg.senderId === curId ? msg.targetId : msg.senderId;
            if (otherId) bumpUserActivity(otherId);
            if (isForCurrentDialog) {
              setMessages((prev) => {
                const exists = prev.some(p => p.id === msg.id);
                return exists ? prev.map(p => (p.id === msg.id ? { ...p, ...msg } : p)) : [...prev, msg];
              });
              refreshUnreadCount();
              refreshSupportUnreadCount();
          ...
          if (envelope?.type === 'support_new_message' && envelope.data) {
            const m = envelope.data;
            const selId = selectedUserIdRef.current;
            // Для support-чатов в списке user.id === chat.id
            if (selId && String(selId) === String(m.chat_id || '')) {
              setMessages((prev) => {
                const exists = prev.some(p => String(p.id) === String(m.id));
                if (exists) return prev.map(p => (String(p.id) === String(m.id) ? { ...p, content: m.content, attachmentUrl: m.attachment_url || '', edited: !!m.edited, senderRole: m.sender_role } : p));
                return [...prev, { id: String(m.id), content: m.content, senderId: m.sender_id, targetId: String(selId), timestamp: m.timestamp, read: true, attachmentUrl: m.attachment_url || '', edited: !!m.edited, senderRole: m.sender_role }];
              });
            } else {
              setSupportUnreadCount((c) => c + 1);
            }
          ...
          if (envelope?.type === 'support_message_update' && envelope.data) {
            const m = envelope.data;
            const selId = selectedUserIdRef.current;
            // Update message if it's in the currently open support chat
            if (selId && String(selId) === String(m.chat_id || '')) {
              setMessages((prev) => prev.map(p => 
                String(p.id) === String(m.id) ? 
                  { ...p, content: m.content, attachmentUrl: m.attachment_url || '', edited: !!m.edited, senderRole: m.sender_role } : 
                  p
              ));
            }
          }
          if (envelope?.type === 'read' && envelope.data) {
            ...
            // Если собеседник (target) сообщил, что прочитал мои сообщения
            const selId = selectedUserIdRef.current; const curId = currentUserIdRef.current;
            if (selId && curId && user_id === curId && by_user_id === selId) {
              setMessages(prev => prev.map(m => (m.senderId === currentUser.id ? { ...m, read: true } : m)));
            }
          }
          // Обработка прочтения support сообщений
          if (envelope?.type === 'support_messages_read' && envelope.data) {
            // Админ прочитал support сообщения - пересчитываем счетчик
            refreshSupportUnreadCount();
          }
          if (envelope?.type === 'message_update' && envelope.data) {
            const { id, content, edited } = envelope.data;
            setMessages(prev => prev.map(m => (String(m.id) === String(id) ? { ...m, content, edited: !!edited } : m)));
          }
          ...
          if (envelope?.type === 'presence' && envelope.data) {
            ...
            setUsers((prev) => prev.map((u) => (u.id === user_id ? { ...u, isOnline: !!online } : u)));
          }
        } catch {}
      };
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentUser, bumpUserActivity]);

  // Шаблоны сообщений для чата поддержки
...
Таким образом, каждый партнер получает персонализированную версию сайта под своим брендом и доменом, при этом используется единая техническая платформа.`
  ...
  // Шаблоны доступны только в чате поддержки (когда админ работает с support чатом)
  const currentTemplates = ((currentUser?.role === 'admin' || currentUser?.role === 'main_admin') && selectedUser?.role === 'support') ? adminTemplates : [];
  ...
  // Removed unused onPickFile function
  ...
  const startEdit = (m) => {
    ...
    try { messageInputRef.current?.focus(); } catch (_) {}
  };
  ...
  const deleteSupportChat = async (chatId) => {
    try {
      ...
      // Закрываем чат и обновляем список пользователей
  ...
  useEffect(() => {
    ...
    const onClick = () => { if (menu.visible) hide(); };
  ...
  return (
    <>
      ...
      {menu.visible && menu.msg && (
        ...
        >
          <button
            ...
            onClick={() => { setMenu({ visible: false, x: 0, y: 0, msg: null }); startEdit(menu.msg); }}
          >
            Изменить
          </button>
          ...
          <button
            ...
            style={{ padding: '8px 12px' }}
            onClick={() => { const id = menu.msg?.id; setMenu({ visible: false, x: 0, y: 0, msg: null }); if (id) setConfirm({ open: true, messageId: id }); }}
          >
            Удалить
          </button>
        </div>
      ...
      <ConfirmModal
        ...
        confirmText="Удалить"
        confirmColor="danger"
        onConfirm={() => { const id = confirm.messageId; setConfirm({ open: false, messageId: null }); if (id) doDeleteMessage(id); }}
        onCancel={() => setConfirm({ open: false, messageId: null })}
        containerSelector=".chat-window"
      />
      <ChatConfirmDialog
        isOpen={!!confirmBlock.open}
        title={confirmBlock.mode === 'block' ? t('chat.block_partner') : t('chat.unblock_partner')}
        message={confirmBlock.mode === 'block' ? t('chat.partner_blocked_message') : t('chat.partner_unblocked_message')}
        confirmText={confirmBlock.mode === 'block' ? 'Заблокировать' : 'Разблокировать'}
        ...
        onConfirm={async () => {
          ...
          try {
            if (confirmBlock.mode === 'block') {
              ...
              // Синхронизируем статус партнёра и его купоны через админ API
              await api.patch(`/admin/partners/${encodeURIComponent(id)}/block`);
              setSelectedUser({ ...selectedUser, status: 'blocked', is_blocked_in_chat: true });
            } else {
              await api.patch(`/chat/partners/${encodeURIComponent(id)}/unblock`);
              await api.patch(`/admin/partners/${encodeURIComponent(id)}/unblock`);
              setSelectedUser({ ...selectedUser, status: 'active', is_blocked_in_chat: false });
            }
            fetchUsers();
          } catch (_) {}
        }}
        ...
        >
          {isOpen ? (
            ...
            <div style={{ position: 'relative' }}>
              ...
              {/* Optional: show split counts tooltip */}
              {unreadCount > 0 && (
                <span title={`Партнёры: ${partnerUnreadCount} • Support: ${supportUnreadCount}`} style={{ position: 'absolute', top: 12, right: -10, fontSize: 9, color: '#999' }}></span>
              )}
            </div>
          )}
        </button>
        ...
        <div className="chat-window" style={{
          ...
          <div className="chat-header" style={{
            ...
            <div className="d-flex justify-content-between align-items-center">
              <h6 className="mb-0">
                {selectedUser ? `${t('chat.chat_with')} ${selectedUser.name}` : t('chat.support_chat')}
              </h6>
              <button
          ...
          {!selectedUser && (
            <div className="chat-users" style={{ flex: 1, overflowY: 'auto', padding: '15px' }}>
              <h6 className="mb-3">
                                 {(currentUser.role === 'admin' || currentUser.role === 'main_admin') ? t('chat.partners') : t('chat.administrators')}
              </h6>
              <div className="mb-3">
                <input
                  type="text"
                  className="form-control"
                                     placeholder={(currentUser.role === 'admin' || currentUser.role === 'main_admin') ? t('chat.search_by_code_brand_email') : t('chat.search_by_email_login')}
                  value={searchTerm}
              ...
              ) : ((String(searchTerm).trim() === '' ? allUsers : users).length === 0) ? (
                <div className="text-center text-muted">
                  <i className="pe-7s-users" style={{ fontSize: '48px', opacity: 0.3 }}></i>
                                     <div className="mt-2">{(currentUser.role === 'admin' || currentUser.role === 'main_admin') ? t('chat.no_partners_available') : t('chat.no_administrators_available')}</div>
                </div>
              ) : (
                <div className="list-group list-group-flush">
                  {(String(searchTerm).trim() === '' ? allUsers : users).map((user) => (
                    <div
                      key={user.id}
                      ...
                      style={{ 
                        ...
                        transition: 'background-color 0.2s ease',
                                                 backgroundColor: ((currentUser.role === 'admin' || currentUser.role === 'main_admin') && user.role === 'partner' && user.is_blocked_in_chat) ? '#fff3cd' : undefined
                      }}
                    >
                      <div 
                      ...
                      >
                        ...
                        <div style={{ flex: 1, marginLeft: 6, lineHeight: 1.1 }}>
                          <div className="font-weight-bold" style={{ marginBottom: 0, display:'flex', alignItems:'center', gap:8 }}>
                            {user.name}
                            {user.role === 'partner' && ((typeof user.status === 'string' && user.status.toLowerCase().includes('block')) || user.is_blocked_in_chat) && (
                              <span className="badge badge-danger">{t('chat.blocked')}</span>
                            )}
                          </div>
                          <small className="text-muted" style={{ display: 'block', marginTop: 2 }}>
                            {user.email}
                            {user.partnerCode ? ` · Код: ${user.partnerCode}` : ''}
                            {!user.partnerCode && user.login ? ` · ${user.login}` : ''}
                          </small>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginLeft: 'auto' }}>
                        {unreadBySender[user.id] > 0 && (
                          <span style={{
                            ...
                            minWidth: 18,
                            textAlign: 'center'
                          }}>{unreadBySender[user.id] > 99 ? '99+' : unreadBySender[user.id]}</span>
                        )}
                                                 {(currentUser.role === 'admin' || currentUser.role === 'main_admin') && user.role === 'support' && (
                          <button 
                            className="btn btn-sm btn-danger" 
                            onClick={(e) => {
                              e.stopPropagation();
                              setConfirmChat({ open: true, chatId: user.id });
                            }}
                            title={t('chat.delete_chat')}
          ...
          {selectedUser && (
            <>
              ...
              <div className="chat-user-header" style={{
                ...
                <div className="d-flex align-items-center">
                  ...
                  <div style={{ marginLeft: 6, lineHeight: 1.1 }}>
                    <div className="font-weight-bold" style={{ marginBottom: 0, display:'flex', alignItems:'center', gap:8 }}>
                      {selectedUser.name}
                                             {(currentUser.role === 'admin' || currentUser.role === 'main_admin') && selectedUser.role === 'partner' && (String(selectedUser.status||'').toLowerCase().includes('block') || selectedUser.is_blocked_in_chat) && (
                        <span className="badge badge-danger">заблокирован</span>
                      )}
                    </div>
                    <small className="text-muted" style={{ display: 'block', marginTop: 2 }}>{selectedUser.email}</small>
                  </div>
                </div>
                <div className="d-flex align-items-center" style={{ gap: 8 }}>
                                     {(currentUser.role === 'admin' || currentUser.role === 'main_admin') && selectedUser.role === 'partner' && (
                    <>
                      {(selectedUser?.is_blocked_in_chat || String(selectedUser?.status || '').toLowerCase().includes('block')) ? (
                        <button className="btn btn-sm btn-success" onClick={() => setConfirmBlock({ open: true, mode: 'unblock' })}>{t('chat.unblock')}</button>
                      ) : (
                        <button className="btn btn-sm btn-warning" onClick={() => setConfirmBlock({ open: true, mode: 'block' })}>{t('chat.block')}</button>
                      )}
                      {/* Кнопка удаления чата убрана: каскадное удаление при удалении партнера */}
                    </>
                  )}
                                     {(currentUser.role === 'admin' || currentUser.role === 'main_admin') && selectedUser.role === 'support' && (
                    <button 
                      className="btn btn-sm btn-danger" 
                      onClick={() => setConfirmChat({ open: true, chatId: selectedUser.id })}
                      title={t('chat.delete_chat')}
                    >
                      <i className="pe-7s-trash"></i>
                    </button>
              ...
              <div
                ...
                onMouseLeave={() => { /* если тянем выделение за пределы контейнера — продолжаем автоскролл */ if (selectingRef.current) { /* noop */ } }}
                style={{ 
                flex: 1, 
                overflowY: 'auto', 
                padding: '15px',
                  ...
                  messages.map((message) => {
                    ...
                    return (
                      <div
                        key={message.id}
                        className="mb-3"
                        style={{ display: 'flex', justifyContent: isMyMessage ? 'flex-end' : 'flex-start' }}
                        onContextMenu={(e) => openContextMenu(e, message)}
                    >
                      <div className={`d-inline-block p-3 rounded ${
                        isMyMessage ? 'bg-primary text-white' : 'bg-white'
                      }`} style={{ 
                        maxWidth: '80%',
                          ...
                          <>
                            <div style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                              {(() => {
                                ...
                                const isDuplicate = !!attachmentUrl && !!displayName && contentText.toLowerCase() === displayName.toLowerCase();
                                return (
                                  <>
                                    {!isDuplicate && contentText && <span>{message.content}</span>}
                                    {attachmentUrl && (
                                      <div className={contentText && !isDuplicate ? 'mt-2' : ''}>
                                        <a
                                          href={attachmentUrl}
                                          target="_blank"
                                          rel="noreferrer"
                                          style={{ color: isMyMessage ? '#ffffff' : '#0d6efd' }}
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
                              ...
                              {isMyMessage && (
                                <small
                                  ...
                                  title={message.read ? 'Прочитано' : 'Отправлено'}
              ...
              <div className="chat-input" style={{ 
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', minHeight: '44px' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', marginRight: 8, marginTop: '2px' }}>
                    <button
                      type="button"
                  ...
                  <textarea
                    ...
                    rows={1}
                  />
                  <div style={{ marginLeft: 8, position: 'relative', display: 'flex', alignItems: 'flex-start', gap: 8, marginTop: '2px' }}>
                    ...
                    <button
                      ...
                      onClick={sendMessage}
                      disabled={(!newMessage.trim() && selectedFiles.length === 0) || !!selectedUser?.is_blocked_in_chat}
                      style={{ borderRadius: 12 }}
                    >
                      {t('chat.send')}
                    </button>
                    {templatesOpen && (
                      ...
                      >
                        <div style={{ fontWeight: 600, padding: '4px 8px', borderBottom: '1px solid #f1f3f5', marginBottom: 6 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                          {currentTemplates.map((t, i) => (
                            <button
                              ...
                              style={{ whiteSpace: 'pre-wrap', textAlign: 'left', borderRadius: 10 }}
                ...
                {selectedFiles.length > 0 && (
                  <div className="mt-2" style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                    {selectedFiles.map((f, idx) => (
                      <div key={idx} className="d-flex align-items-center" style={{ gap: 8, background: '#f1f3f5', borderRadius: 8, padding: '4px 8px' }}>
                        <small style={{ maxWidth: 260, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{f.name}</small>
                        <button className="btn btn-sm btn-link text-danger p-0" onClick={() => removeFileAt(idx)} title="Убрать">×</button>
                      </div>
                    ))}
                  </div>
                )}
