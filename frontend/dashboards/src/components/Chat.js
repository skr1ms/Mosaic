...
const Chat = () => {
  ...
  }, [currentUser, searchTerm, reorderByActivity]);
  ...
  // Шаблоны сообщений (вопросы для партнера, ответы для админа)
...
Таким образом, каждый партнер получает персонализированную версию сайта под своим брендом и доменом, при этом используется единая техническая платформа.`
     ...
     const currentTemplates = (currentUser?.role === 'admin' || currentUser?.role === 'main_admin') ? adminTemplates : partnerTemplates;
                    ...
                    {templatesOpen && (
                      ...
                      >
                        <div style={{ fontWeight: 600, padding: '4px 8px', borderBottom: '1px solid #f1f3f5', marginBottom: 6 }}>
                                                     {(currentUser?.role === 'admin' || currentUser?.role === 'main_admin') ? 'Шаблоны ответов' : 'Шаблоны вопросов'}
                        </div>
