...
setup_webroot() {
    ...
    if docker ps --format "table {{.Names}}" | grep -q "nginx" 2>/dev/null; then
        docker exec nginx mkdir -p "$WEBROOT_PATH" 2>/dev/null
        docker exec nginx chown -R nginx:nginx "$WEBROOT_PATH" 2>/dev/null || true
        log "✅ Webroot directory ready: $WEBROOT_PATH"
        return 0
    else
...
generate_certificate() {
    ...
    # Certbot command
    local certbot_cmd="certonly --webroot --webroot-path=$WEBROOT_PATH --email $CERTBOT_EMAIL --agree-tos --no-eff-email --cert-name $CERT_NAME"
    
    if [ "$is_renewal" = "true" ]; then
        certbot_cmd="$certbot_cmd --expand"
        log "📱 Expanding existing certificate with new domains"
    else
    ...
    if docker run --rm --name certbot \
        ...
        -v "/var/www/certbot:$WEBROOT_PATH" \
